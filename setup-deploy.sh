...
init_colors() {
  if [ -t 1 ] && command -v tput >/dev/null 2>&1 \
    ...
    CC="$(tput setaf 6 2>/dev/null || printf '')"   # cyan
    CG="$(tput setaf 2 2>/dev/null || printf '')"   # green
    CY="$(tput setaf 3 2>/dev/null || printf '')"   # yellow
    CR="$(tput setaf 1 2>/dev/null || printf '')"   # red
    CM="$(tput setaf 5 2>/dev/null || printf '')"   # magenta
    CW="$(tput setaf 7 2>/dev/null || printf '')"   # white
  fi
}
...
_measure() {
  ...
  # Double-line box chars for premium look
  local r
...
_fmt_row_progress() {
  ...
  printf '║  %s%s%s%s%s  ║' "${CB}${CC}" "$fbar" "${C0}${CD}" "$ebar" "$C0"
}
...
_row_accent()   { _emit "$(_fmt_row_accent   "${1:-›}" "${2:-}" "${3:-$CC}")"; }
...
_info() { printf '  %s◆%s  %s\n'  "${CC}" "$C0" "$1" >/dev/tty; }
_ok()   { printf '  %s✔%s  %s%s%s\n' "${CB}${CG}" "$C0" "$CG" "$1" "$C0" >/dev/tty; }
_warn() { printf '  %s⚠%s  %s%s%s\n' "${CB}${CY}" "$C0" "$CY" "$1" "$C0" >/dev/tty; }
...
_pause() {
  printf '\n  %s%s↵  Press Enter to continue…%s ' "$CD" "$CB" "$C0" >/dev/tty
  _cur_show
...
prompt_input() {
  ...
  while true; do
    ...
    printf '  %s%s⚠%s  Value cannot be empty.\n' "$CB" "$CY" "$C0" >/dev/tty
  done
}
...
prompt_yes_no() {
  ...
  while true; do
    ...
    if [ -n "$def" ]; then
      if [ "$def" = "yes" ]; then
        printf ' %s[%sYes%s/%sno%s]%s' "$CD" "${CB}${CG}" "$C0${CD}" "" "" "$C0" >/dev/tty
      else
        printf ' %s[%syes%s/%sNo%s]%s' "$CD" "" "" "${CB}${CR}" "$C0${CD}" "$C0" >/dev/tty
      fi
    fi
    printf ': ' >/dev/tty
    _cur_show
    ...
    case "$lv" in
      ...
      *) printf '  %s%s⚠%s  Please enter %syes%s or %sno%s.\n' \
           "$CB" "$CY" "$C0" "$CB" "$C0" "$CB" "$C0" >/dev/tty ;;
    esac
  done
}
...
prompt_choice() {
  ...
  _REPLY=""
  printf '\n  %s❯%s  %sChoice%s %s[%s]%s: ' \
    "${CB}${CC}" "$C0" "$CB" "$C0" "$CD" "$def" "$C0" >/dev/tty
  _cur_show
  IFS= read -r val </dev/tty
  _cur_hide
  [ -z "$val" ] && val="$def"
...
show_splash() {
  ...
  # ASCII art logo — 5 lines tall, always centered
  local logo_w=44   # visual width of the widest logo line
  local lpad=$(( (BOX_W - logo_w) / 2 ))
  [ "$lpad" -lt 0 ] && lpad=0
  local indent="$(printf '%*s' "$lpad" '')"

  local top_pad=$(( (th - 22) / 2 ))
  ...
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$(_fmt_row_center "Self-hosted · Production-ready · Open source")" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_row_center "  ↵  Press Enter to begin  " "${CB}${CG}" "$C0")" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_BOT" >/dev/tty
...
check_prereqs() {
  ...
  _blank
  _row_bold "Verifying required tools before setup begins."
  _blank
  _bot
  printf '\n' >/dev/tty

  require_cmd docker;  _ok "docker found"
  require_cmd unzip;   _ok "unzip found"
...
review_and_confirm() {
  while true; do
    ...
    _print_summary
    _blank
    _row_accent "1" "Deploy now" "$CG"
    _row_accent "2" "Edit domain configuration" "$CC"
    _row_accent "3" "Edit database settings" "$CC"
    _row_accent "4" "Edit docker mode and ports" "$CC"
    _row_accent "5" "Edit public ports only" "$CC"
    _row_accent "6" "Edit backup restore" "$CC"
    _row_accent "7" "Exit without changes" "$CD"
    _bot
...
maintenance_menu_if_running() {
  ...
  while true; do
    ...
    _blank
    _row_bold "$DETECTED_UP_DETAILS"
    _row_dim "Mode: $MODE"
    [ "$MODE" = "swarm" ] && _row_dim "Stack: $STACK_NAME"
    _mid
    _row_accent "1" "Continue with full deployment wizard" "$CG"
    _row_accent "2" "Update database credentials" "$CC"
    _row_accent "3" "Rebuild / update app" "$CC"
    _row_accent "4" "Check GitHub updates (origin/main) and download" "$CY"
    _row_accent "5" "Edit environment variables" "$CC"
    _row_accent "6" "View Anthropic API usage" "$CM"
    _row_accent "7" "Exit" "$CD"
    _bot
...
show_completion() {
  ...
  local cel_art="★  ★  ★  ★  ★  ★  ★  ★  ★  ★"
  printf '%s\n' "$(_fmt_row_center "$cel_art" "${CB}${CY}" "$C0")" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty

  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "App URL:"   "${base}/")" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Setup URL:" "${base}/setup")" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Admin URL:" "${base}/admin")" >/dev/tty
  if [ -n "$DOMAIN" ]; then
    printf '%s\n' "$(_fmt_blank)" >/dev/tty
    printf '%s\n' "$(_fmt_row_kv "Domain:" "$DOMAIN")" >/dev/tty
    [ "$ALLOW_SUBDOMAINS" = "yes" ] && printf '%s\n' "$(_fmt_row_kv "Subdomains:" "*.${DOMAIN}  (wildcard)")" >/dev/tty
    [ -n "$EXTRA_DOMAINS" ]         && printf '%s\n' "$(_fmt_row_kv "Extra:"      "$EXTRA_DOMAINS")" >/dev/tty
  fi
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_row_dim "First visit /setup to create your admin account.")" >/dev/tty
  if [ -z "$DOMAIN" ]; then
    printf '%s\n' "$(_fmt_row_dim "Point a domain to this server and re-run setup to configure it.")" >/dev/tty
  else
    printf '%s\n' "$(_fmt_row_dim "DNS must resolve $DOMAIN to this server's IP address.")" >/dev/tty
    printf '%s\n' "$(_fmt_row_dim "Add TLS with Let's Encrypt (certbot) when ready for production.")" >/dev/tty
  fi
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_BOT" >/dev/tty

  printf '\n  %s%s↵  Press Enter to exit…%s ' "$CB" "$CD" "$C0" >/dev/tty
  _cur_show
  IFS= read -r _ </dev/tty 2>/dev/null || true
  _cur_hide
}
...
# Silently checks for updates from origin/main. If behind, offers the user
# a chance to update before proceeding. Non-blocking on network failure.

_startup_update_check() {
  # Require git and a valid repo with an 'origin' remote.
  _git_root_check 2>/dev/null || return 0
  local remote_url
  remote_url="$(git -C "$ROOT_DIR" remote get-url origin 2>/dev/null || true)"
  [ -n "$remote_url" ] || return 0

  local branch behind remote_sha local_sha
  branch="$(_git_current_branch)"
  [ -n "$branch" ] || return 0

  # Silent fetch — if network is unavailable, just continue
  _measure
  _cls
  printf '\n%s\n' "$_BOX_TOP" >/dev/tty
  printf '%s\n' "$(_fmt_row_logo)" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_row_center "Checking for updates…" "${CD}" "$C0")" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_BOT" >/dev/tty

  git -C "$ROOT_DIR" fetch origin main --quiet >/dev/null 2>&1 || { _cls; return 0; }
  ...
  # No updates — proceed silently
  [ "${behind:-0}" -eq 0 ] && return 0

  # Updates available — prompt the user
  _measure
  printf '\n%s\n' "$_BOX_TOP" >/dev/tty
  printf '%s\n' "$(_fmt_row_logo)" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_row_center "  ★  Update Available  ★  " "${CB}${CY}" "$C0")" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Commits behind:" "$behind")" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Remote:" "$remote_url")" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Branch:" "$branch")" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Current SHA:"  "${local_sha:0:12}")" >/dev/tty
  printf '%s\n' "$(_fmt_row_kv "Available SHA:" "${remote_sha:0:12}")" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_MID" >/dev/tty
  printf '%s\n' "$(_fmt_row_dim "Apply update now? Stashes local changes, pulls, then reapplies.")" >/dev/tty
  printf '%s\n' "$(_fmt_blank)" >/dev/tty
  printf '%s\n' "$_BOX_BOT" >/dev/tty

  prompt_yes_no "Apply update now?" "yes"
  if [ "$_REPLY" = "yes" ]; then
    _cls
    printf '\n' >/dev/tty
    if _git_is_dirty; then
      _do_git_pull_with_stash || true
    else
      _do_git_pull_clean || true
    fi
    _ok "Update complete. Continuing to deployment wizard…"
    _pause
  fi
}
